...
class AtomDiffusion(Module):
    ...
    def sample(
        ...
        **network_condition_kwargs,
        ...
        gammas = torch.zeros_like(sigmas)  # remove churn to make process deteministic
        ...
        for step_idx, (sigma_tm, sigma_t, gamma) in enumerate(sigmas_and_gammas):
            ...
            # ODE mode: no churn, no per-step noise
            t_hat = sigma_tm  # since gamma==0
            steering_t = 1.0 - (step_idx / num_sampling_steps)
            atom_coords_noisy = atom_coords  # eps = 0, noise_var = 0
